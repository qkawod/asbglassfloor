...
export default function ReferencesPage() {
    ...
    return (
        <div className="min-h-screen bg-[#111] text-white font-sans selection:bg-neonCyan selection:text-black">
            ...
            {/* 2. Filter System (Restored) */}
            <section className="py-12 px-6 sticky top-0 z-40 bg-[#111]/90 backdrop-blur-md border-b border-white/10">
                <div className="container mx-auto max-w-[1700px] flex flex-wrap gap-4 items-center justify-center md:justify-start">
                    {categories.map((cat) => (
                        <button
                            key={cat}
                            onClick={() => setActiveFilter(cat)}
                            className={`
                        px-8 py-3 text-sm font-bold tracking-widest transition-all duration-300 outline-none focus:outline-none
                        ${activeFilter === cat
                                    ? "text-white [text-shadow:0_0_15px_#FFE57F] scale-105"
                                    : "text-gray-400 hover:text-white hover:[text-shadow:0_0_10px_#FFE57F]"
                                }
                    `}
                        >
                            {cat}
            ...
            {/* Using strict custom styles matching request */}
            <section className="py-[40px] px-[20px] bg-[#111]">
                        ...
                        {filteredProjects.map((project, index) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    {/* Content Wrapper for Hover Animation */}
                                    <div className="relative z-20 flex flex-col items-start transform transition-transform duration-500 cubic-bezier(0.25, 0.46, 0.45, 0.94) group-hover:-translate-y-[10px)">
                                        ...
                                        <div className="inline-block mb-[12px]">
                                            ...
                                            >
                                                {project.type === "L" ? "풀LED 스마트코트" : "ASB 스마트코트"}
                                            </span>
                                        </div>
